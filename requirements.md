...
Extensions: FTS5 (Full-Text Search), R-Tree (Optional for future GIS)

Settings: PRAGMA foreign_keys = ON; (常時必須)
...
subfamily, body_len_min, body_len_max, dist_text, elev_min, elev_max, red_list
...
units: 'worker', 'colony', 'queen' などを管理。
...
※ スニペット生成のため contentless オプションは使用しない。
...
※ 同一地点・同一種でも、採集法や単位が異なれば別レコード。

4. データインポート (ETL) 要件
4.1 全体方針
Idempotency (冪等性): スクリプトは何度実行しても安全であること。

Log: エラー行は error_log.csv に出力し、プロセスを停止させない（スキップ処理）。

Normalization: 全ての入力文字列に対し、unicodedata.normalize('NFKC', text) を適用してからDB照合を行う。
...
02_research.csv: 論文情報。txt_file列がある場合はファイル読み込み。
